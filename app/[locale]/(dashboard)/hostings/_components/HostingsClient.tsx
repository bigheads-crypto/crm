...
export function HostingsClient({ initialData, initialCount, role }: Props) {
  ...
  useEffect(() => {
    async function loadOptions() {
      ...
      const { data: p } = await supabase.from('hostings').select('provider').not('provider', 'is', null)
  ...
  return (
    <>
      <div className="mb-6">
        <h2 className="text-xl font-bold" style={{ color: 'var(--text)' }}>Hostingi</h2>
        <p className="text-sm mt-0.5" style={{ color: 'var(--text-muted)' }}>
          Zarządzanie hostingami i serwerami
        </p>
      </div>
      <DataTable
